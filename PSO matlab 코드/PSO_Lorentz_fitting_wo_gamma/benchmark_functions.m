...
%Adopted from: S. Mirjalili, S.Z. Mohd Hashim, 밃 New Hybrid PSOGSA 
...
function fit=benchmark_functions(L,Benchmark_Function_ID,dim,FileName)
% L = current position 
...
if Benchmark_Function_ID==25
   %fit= QCRF_Coef(L);
   % fit= QCRF(L);
   fit=QCRF_linear(L,FileName);
end
if Benchmark_Function_ID==26
   %fit= QCRF_Coef(L);
   % fit= QCRF(L);
   %fit=QCRF_linear(L);
   fit=Ag_drude(L);
end
if Benchmark_Function_ID==27
   %fit= QCRF_Coef(L);
   % fit= QCRF(L);
   fit=QCRF_linear_non_negative(L,FileName);
end
%% 위치, 파일데이터 넣고 Lorentz 모델 돌리기
if Benchmark_Function_ID==28
   %fit= QCRF_Coef(L);
   % fit= QCRF(L);
   fit=MEEP_unit_Lorentz(L,FileName); % Fitness 함수
end
