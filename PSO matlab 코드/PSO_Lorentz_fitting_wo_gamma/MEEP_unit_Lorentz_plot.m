...
for m = 1 : length(freq)
    ...
    pole = 0;
%
    for k = 1 : length(gamma)/2
        eps(m) = eps(m) +  gamma(1 + pole*2)^2 * gamma(2 + pole*2)/(gamma(1 + pole*2)^2 - freq(m)^2 );%- 1*j*freq(m)*gamma(2 + pole*3));
        pole = pole + 1;
    end
end
...
% 피팅결과 (실선)
plot(1./freq.*1e-6./1e-9,real(eps),'r-','linewidth',2);
% plot(freq,real(eps),'r-','linewidth',2);
hold on

% 소재 데이터 (동그라미)
plot(1./freq.*1e-6./1e-9,eps1,'ro','linewidth',2);
...
% 피팅결과 (실선)
 plot(1./freq.*1e-6./1e-9,imag(eps),'b-','linewidth',2);
% plot(freq,imag(eps),'b-','linewidth',2);
hold on

% 소재 데이터 (동그라미)
 plot(1./freq.*1e-6./1e-9,eps2,'bo','linewidth',2);
