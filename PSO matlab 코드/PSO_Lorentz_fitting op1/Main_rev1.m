...
%% 소재 데이터 선택 
FileName = ['TiO2_meep_f_eps1_eps2.txt'];
...
%para_QCRF_curve(gBest);
gBestScore_Matrix(i,:)=gBestScore;
%QCRF_Coef(best_fit_position(Max_Iteration,:));
end
...
%% 1열기준 오름차순 정렬: 제일 낮은 Fitness값(오차)을가진 행으로 구성된 1행짜리행렬 생성 
Post_processing=sortrows(Post_processing,1);
Gamma = zeros(1,dim);
%% 거기있는 gamma(gbest값)들 전부다 Gamma(제일 잘 뽑힌거)에 때려넣기
for i = 1 : dim
    Gamma(i) = Post_processing(1,i+1);
end
%% 그래프 그리기 및 오차 계산
MEEP_unit_Lorentz_plot(Gamma,FileName);
%QCRF_curve_linear_ext(Gamma,FileName);