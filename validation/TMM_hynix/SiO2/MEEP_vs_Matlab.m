...
f0_SiO2_meep = [10.469053074021327, 10.4149782511721946];
gamma_SiO2_meep = [0.0, 0.0, 0.0]; % it is already devided by 2pi
sigma_SiO2_meep = [0.4149782511721946, 0.6411115779816661];

eps_SiO2_meep = 1+ (sigma_SiO2_meep(1)*f0_SiO2_meep(1)^2)./(f0_SiO2_meep(1)^2 - f.^2 - 1i*f.*gamma_SiO2_meep(1));
eps_SiO2_meep = eps_SiO2_meep + (sigma_SiO2_meep(2)*f0_SiO2_meep(2)^2)./(f0_SiO2_meep(2)^2 - f.^2 - 1i*f.*gamma_SiO2_meep(2));
...
%     SiN_eps = @(w)(real(eps_sin_meep(k)) + imag(eps_sin_meep(k))*1.0i);
    SiO2_eps = @(w)(real(eps_SiO2_meep (k)) + imag(eps_SiO2_meep (k))*1.0i);
