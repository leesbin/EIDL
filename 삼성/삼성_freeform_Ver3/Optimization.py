...
###########################################################################
    ...
    if True: #####  Set wavelengths and sources  ##################################################################################################################
        wavelengths = np.linspace(0.4*um_scale, 0.7*um_scale, 61)
        wavelengths_n = np.array([
                wavelengths[6], wavelengths[8], wavelengths[10], wavelengths[12], wavelengths[14],
                wavelengths[26], wavelengths[28], wavelengths[30], wavelengths[32], wavelengths[34],
                wavelengths[46], wavelengths[48], wavelengths[50], wavelengths[52], wavelengths[54]
        ])
        frequencies = 1/wavelengths
        frequencies_n = 1/wavelengths_n
...
######################################################################################################
    ...
    if ML: # Multi Design Region  #################################################################################################################################
        ...
        design_region = mpa.DesignRegion(
            design_variables,
            volume=mp.Volume(
                center=mp.Vector3(0, 0, round(Sz / 2 - Lpml -  pml_2_src - src_2_geo - design_region_height/2, 2)),
    ...
    if True: ##### Set geometries #################################################################################################################################
        if ML:
            ...
            geometry = [
                ...
                # SiPD
                mp.Block(
                    center=mp.Vector3(0, 0, round(Sz / 2 - Lpml -  pml_2_src - src_2_geo - ar_thktop - design_region_height - fl_thickness - ar_thkbottom - mon_2_pml/2 - Lpml/2, 2)), size=mp.Vector3(Sx, Sy, mon_2_pml + Lpml), material=SiO2
                ),
                # design region과 동일한 size의 Block 생성
                mp.Block(
    ...
    if Norm: ##### normalization flux calculation #################################################################################################################
        ...
        b_0_tran_Ex = sim.add_dft_fields([mp.Ex], frequencies_n[0], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        b_0_tran_Ey = sim.add_dft_fields([mp.Ey], frequencies_n[0], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        b_0_tran_Ez = sim.add_dft_fields([mp.Ez], frequencies_n[0], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        b_0_tran_Hx = sim.add_dft_fields([mp.Hx], frequencies_n[0], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        b_0_tran_Hy = sim.add_dft_fields([mp.Hy], frequencies_n[0], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        b_0_tran_Hz = sim.add_dft_fields([mp.Hz], frequencies_n[0], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        b_1_tran_Ex = sim.add_dft_fields([mp.Ex], frequencies_n[1], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        b_1_tran_Ey = sim.add_dft_fields([mp.Ey], frequencies_n[1], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        b_1_tran_Ez = sim.add_dft_fields([mp.Ez], frequencies_n[1], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        b_1_tran_Hx = sim.add_dft_fields([mp.Hx], frequencies_n[1], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        b_1_tran_Hy = sim.add_dft_fields([mp.Hy], frequencies_n[1], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        b_1_tran_Hz = sim.add_dft_fields([mp.Hz], frequencies_n[1], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        b_2_tran_Ex = sim.add_dft_fields([mp.Ex], frequencies_n[2], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        b_2_tran_Ey = sim.add_dft_fields([mp.Ey], frequencies_n[2], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        b_2_tran_Ez = sim.add_dft_fields([mp.Ez], frequencies_n[2], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        b_2_tran_Hx = sim.add_dft_fields([mp.Hx], frequencies_n[2], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        b_2_tran_Hy = sim.add_dft_fields([mp.Hy], frequencies_n[2], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        b_2_tran_Hz = sim.add_dft_fields([mp.Hz], frequencies_n[2], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        b_3_tran_Ex = sim.add_dft_fields([mp.Ex], frequencies_n[3], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        b_3_tran_Ey = sim.add_dft_fields([mp.Ey], frequencies_n[3], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        b_3_tran_Ez = sim.add_dft_fields([mp.Ez], frequencies_n[3], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        b_3_tran_Hx = sim.add_dft_fields([mp.Hx], frequencies_n[3], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        b_3_tran_Hy = sim.add_dft_fields([mp.Hy], frequencies_n[3], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        b_3_tran_Hz = sim.add_dft_fields([mp.Hz], frequencies_n[3], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        b_4_tran_Ex = sim.add_dft_fields([mp.Ex], frequencies_n[4], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        b_4_tran_Ey = sim.add_dft_fields([mp.Ey], frequencies_n[4], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        b_4_tran_Ez = sim.add_dft_fields([mp.Ez], frequencies_n[4], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        b_4_tran_Hx = sim.add_dft_fields([mp.Hx], frequencies_n[4], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        b_4_tran_Hy = sim.add_dft_fields([mp.Hy], frequencies_n[4], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        b_4_tran_Hz = sim.add_dft_fields([mp.Hz], frequencies_n[4], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        g_0_tran_Ex = sim.add_dft_fields([mp.Ex], frequencies_n[5], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        g_0_tran_Ey = sim.add_dft_fields([mp.Ey], frequencies_n[5], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        g_0_tran_Ez = sim.add_dft_fields([mp.Ez], frequencies_n[5], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        g_0_tran_Hx = sim.add_dft_fields([mp.Hx], frequencies_n[5], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        g_0_tran_Hy = sim.add_dft_fields([mp.Hy], frequencies_n[5], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        g_0_tran_Hz = sim.add_dft_fields([mp.Hz], frequencies_n[5], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        g_1_tran_Ex = sim.add_dft_fields([mp.Ex], frequencies_n[6], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        g_1_tran_Ey = sim.add_dft_fields([mp.Ey], frequencies_n[6], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        g_1_tran_Ez = sim.add_dft_fields([mp.Ez], frequencies_n[6], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        g_1_tran_Hx = sim.add_dft_fields([mp.Hx], frequencies_n[6], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        g_1_tran_Hy = sim.add_dft_fields([mp.Hy], frequencies_n[6], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        g_1_tran_Hz = sim.add_dft_fields([mp.Hz], frequencies_n[6], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        g_2_tran_Ex = sim.add_dft_fields([mp.Ex], frequencies_n[7], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        g_2_tran_Ey = sim.add_dft_fields([mp.Ey], frequencies_n[7], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        g_2_tran_Ez = sim.add_dft_fields([mp.Ez], frequencies_n[7], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        g_2_tran_Hx = sim.add_dft_fields([mp.Hx], frequencies_n[7], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        g_2_tran_Hy = sim.add_dft_fields([mp.Hy], frequencies_n[7], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        g_2_tran_Hz = sim.add_dft_fields([mp.Hz], frequencies_n[7], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        g_3_tran_Ex = sim.add_dft_fields([mp.Ex], frequencies_n[8], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        g_3_tran_Ey = sim.add_dft_fields([mp.Ey], frequencies_n[8], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        g_3_tran_Ez = sim.add_dft_fields([mp.Ez], frequencies_n[8], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        g_3_tran_Hx = sim.add_dft_fields([mp.Hx], frequencies_n[8], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        g_3_tran_Hy = sim.add_dft_fields([mp.Hy], frequencies_n[8], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        g_3_tran_Hz = sim.add_dft_fields([mp.Hz], frequencies_n[8], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        g_4_tran_Ex = sim.add_dft_fields([mp.Ex], frequencies_n[9], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        g_4_tran_Ey = sim.add_dft_fields([mp.Ey], frequencies_n[9], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        g_4_tran_Ez = sim.add_dft_fields([mp.Ez], frequencies_n[9], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        g_4_tran_Hx = sim.add_dft_fields([mp.Hx], frequencies_n[9], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        g_4_tran_Hy = sim.add_dft_fields([mp.Hy], frequencies_n[9], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        g_4_tran_Hz = sim.add_dft_fields([mp.Hz], frequencies_n[9], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        r_0_tran_Ex = sim.add_dft_fields([mp.Ex], frequencies_n[10], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        r_0_tran_Ey = sim.add_dft_fields([mp.Ey], frequencies_n[10], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        r_0_tran_Ez = sim.add_dft_fields([mp.Ez], frequencies_n[10], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        r_0_tran_Hx = sim.add_dft_fields([mp.Hx], frequencies_n[10], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        r_0_tran_Hy = sim.add_dft_fields([mp.Hy], frequencies_n[10], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        r_0_tran_Hz = sim.add_dft_fields([mp.Hz], frequencies_n[10], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        r_1_tran_Ex = sim.add_dft_fields([mp.Ex], frequencies_n[11], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        r_1_tran_Ey = sim.add_dft_fields([mp.Ey], frequencies_n[11], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        r_1_tran_Ez = sim.add_dft_fields([mp.Ez], frequencies_n[11], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        r_1_tran_Hx = sim.add_dft_fields([mp.Hx], frequencies_n[11], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        r_1_tran_Hy = sim.add_dft_fields([mp.Hy], frequencies_n[11], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        r_1_tran_Hz = sim.add_dft_fields([mp.Hz], frequencies_n[11], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        r_2_tran_Ex = sim.add_dft_fields([mp.Ex], frequencies_n[12], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        r_2_tran_Ey = sim.add_dft_fields([mp.Ey], frequencies_n[12], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        r_2_tran_Ez = sim.add_dft_fields([mp.Ez], frequencies_n[12], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        r_2_tran_Hx = sim.add_dft_fields([mp.Hx], frequencies_n[12], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        r_2_tran_Hy = sim.add_dft_fields([mp.Hy], frequencies_n[12], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        r_2_tran_Hz = sim.add_dft_fields([mp.Hz], frequencies_n[12], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        r_3_tran_Ex = sim.add_dft_fields([mp.Ex], frequencies_n[13], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        r_3_tran_Ey = sim.add_dft_fields([mp.Ey], frequencies_n[13], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        r_3_tran_Ez = sim.add_dft_fields([mp.Ez], frequencies_n[13], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        r_3_tran_Hx = sim.add_dft_fields([mp.Hx], frequencies_n[13], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        r_3_tran_Hy = sim.add_dft_fields([mp.Hy], frequencies_n[13], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        r_3_tran_Hz = sim.add_dft_fields([mp.Hz], frequencies_n[13], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        r_4_tran_Ex = sim.add_dft_fields([mp.Ex], frequencies_n[14], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        r_4_tran_Ey = sim.add_dft_fields([mp.Ey], frequencies_n[14], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        r_4_tran_Ez = sim.add_dft_fields([mp.Ez], frequencies_n[14], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)

        r_4_tran_Hx = sim.add_dft_fields([mp.Hx], frequencies_n[14], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        r_4_tran_Hy = sim.add_dft_fields([mp.Hy], frequencies_n[14], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
        r_4_tran_Hz = sim.add_dft_fields([mp.Hz], frequencies_n[14], 0 , 1, center=mp.Vector3(0, 0, round(-Sz/2 + Lpml + 1/resolution,2)), size=mp.Vector3(Sx, Sy, 0), yee_grid=True)
    ...
    if True: ##### Optimization Environment ############################################################################################################################
        ...
        if Norm:
            def J(fields_1_x, fields_1_y, fields_2_x, fields_2_y, fields_3_x, fields_3_y, fields_4_x, fields_4_y, fields_1_x_h, fields_1_y_h, fields_2_x_h, fields_2_y_h, fields_3_x_h, fields_3_y_h, fields_4_x_h, fields_4_y_h):
                red_0 = -npa.sum(fields_1_x[46,:]*(npa.real(fields_1_x_h[46,:])-npa.imag(fields_1_x_h[46,:])*1j)) + npa.sum(fields_1_y[46,:]*(npa.real(fields_1_y_h[46,:])-npa.imag(fields_1_y_h[46,:])*1j))
                red_1 = -npa.sum(fields_1_x[48,:]*(npa.real(fields_1_x_h[48,:])-npa.imag(fields_1_x_h[48,:])*1j)) + npa.sum(fields_1_y[48,:]*(npa.real(fields_1_y_h[48,:])-npa.imag(fields_1_y_h[48,:])*1j))
                red_2 = -npa.sum(fields_1_x[50,:]*(npa.real(fields_1_x_h[50,:])-npa.imag(fields_1_x_h[50,:])*1j))  + npa.sum(fields_1_y[50,:]*(npa.real(fields_1_y_h[50,:])-npa.imag(fields_1_y_h[50,:])*1j))
                red_3 = -npa.sum(fields_1_x[52,:]*(npa.real(fields_1_x_h[52,:])-npa.imag(fields_1_x_h[52,:])*1j)) + npa.sum(fields_1_y[52,:]*(npa.real(fields_1_y_h[52,:])-npa.imag(fields_1_y_h[52,:])*1j))
                red_4 = -npa.sum(fields_1_x[54,:]*(npa.real(fields_1_x_h[54,:])-npa.imag(fields_1_x_h[54,:])*1j)) + npa.sum(fields_1_y[54,:]*(npa.real(fields_1_y_h[54,:])-npa.imag(fields_1_y_h[54,:])*1j))

                CT_red_0 = -npa.sum(fields_1_x[6,:]*(npa.real(fields_1_x_h[6,:])-npa.imag(fields_1_x_h[6,:])*1j)) + npa.sum(fields_1_y[6,:]*(npa.real(fields_1_y_h[6,:])-npa.imag(fields_1_y_h[6,:])*1j))
                CT_red_1 = -npa.sum(fields_1_x[8,:]*(npa.real(fields_1_x_h[8,:])-npa.imag(fields_1_x_h[8,:])*1j)) + npa.sum(fields_1_y[8,:]*(npa.real(fields_1_y_h[8,:])-npa.imag(fields_1_y_h[8,:])*1j))
                CT_red_2 = -npa.sum(fields_1_x[10,:]*(npa.real(fields_1_x_h[10,:])-npa.imag(fields_1_x_h[10,:])*1j)) + npa.sum(fields_1_y[10,:]*(npa.real(fields_1_y_h[10,:])-npa.imag(fields_1_y_h[10,:])*1j))
                CT_red_3 = -npa.sum(fields_1_x[12,:]*(npa.real(fields_1_x_h[12,:])-npa.imag(fields_1_x_h[12,:])*1j)) + npa.sum(fields_1_y[12,:]*(npa.real(fields_1_y_h[12,:])-npa.imag(fields_1_y_h[12,:])*1j))
                CT_red_4 = -npa.sum(fields_1_x[14,:]*(npa.real(fields_1_x_h[14,:])-npa.imag(fields_1_x_h[14,:])*1j)) + npa.sum(fields_1_y[14,:]*(npa.real(fields_1_y_h[14,:])-npa.imag(fields_1_y_h[14,:])*1j))
                CT_red_5 = -npa.sum(fields_1_x[26,:]*(npa.real(fields_1_x_h[26,:])-npa.imag(fields_1_x_h[26,:])*1j)) + npa.sum(fields_1_y[26,:]*(npa.real(fields_1_y_h[26,:])-npa.imag(fields_1_y_h[26,:])*1j))
                CT_red_6 = -npa.sum(fields_1_x[28,:]*(npa.real(fields_1_x_h[28,:])-npa.imag(fields_1_x_h[28,:])*1j)) + npa.sum(fields_1_y[28,:]*(npa.real(fields_1_y_h[28,:])-npa.imag(fields_1_y_h[28,:])*1j))
                CT_red_7 = -npa.sum(fields_1_x[30,:]*(npa.real(fields_1_x_h[30,:])-npa.imag(fields_1_x_h[30,:])*1j)) + npa.sum(fields_1_y[30,:]*(npa.real(fields_1_y_h[30,:])-npa.imag(fields_1_y_h[30,:])*1j))
                CT_red_8 = -npa.sum(fields_1_x[32,:]*(npa.real(fields_1_x_h[32,:])-npa.imag(fields_1_x_h[32,:])*1j)) + npa.sum(fields_1_y[32,:]*(npa.real(fields_1_y_h[32,:])-npa.imag(fields_1_y_h[32,:])*1j))
                CT_red_9 = -npa.sum(fields_1_x[34,:]*(npa.real(fields_1_x_h[34,:])-npa.imag(fields_1_x_h[34,:])*1j)) + npa.sum(fields_1_y[34,:]*(npa.real(fields_1_y_h[34,:])-npa.imag(fields_1_y_h[34,:])*1j))

                green_0 = -npa.sum(fields_2_x[26,:]*(npa.real(fields_2_x_h[26,:])-npa.imag(fields_2_x_h[26,:])*1j)) + npa.sum(fields_2_y[26,:]*(npa.real(fields_2_y_h[26,:])-npa.imag(fields_2_y_h[26,:])*1j)) - npa.sum(fields_4_x[26,:]*(npa.real(fields_4_x_h[26,:])-npa.imag(fields_4_x_h[26,:])*1j)) + npa.sum(fields_4_y[26,:]*(npa.real(fields_4_y_h[26,:])-npa.imag(fields_4_y_h[26,:])*1j))
                green_1 = -npa.sum(fields_2_x[28,:]*(npa.real(fields_2_x_h[28,:])-npa.imag(fields_2_x_h[28,:])*1j)) + npa.sum(fields_2_y[28,:]*(npa.real(fields_2_y_h[28,:])-npa.imag(fields_2_y_h[28,:])*1j)) - npa.sum(fields_4_x[28,:]*(npa.real(fields_4_x_h[28,:])-npa.imag(fields_4_x_h[28,:])*1j)) + npa.sum(fields_4_y[28,:]*(npa.real(fields_4_y_h[28,:])-npa.imag(fields_4_y_h[28,:])*1j))
                green_2 = -npa.sum(fields_2_x[30,:]*(npa.real(fields_2_x_h[30,:])-npa.imag(fields_2_x_h[30,:])*1j)) + npa.sum(fields_2_y[30,:]*(npa.real(fields_2_y_h[30,:])-npa.imag(fields_2_y_h[30,:])*1j)) - npa.sum(fields_4_x[30,:]*(npa.real(fields_4_x_h[30,:])-npa.imag(fields_4_x_h[30,:])*1j)) + npa.sum(fields_4_y[30,:]*(npa.real(fields_4_y_h[30,:])-npa.imag(fields_4_y_h[30,:])*1j))
                green_3 = -npa.sum(fields_2_x[32,:]*(npa.real(fields_2_x_h[32,:])-npa.imag(fields_2_x_h[32,:])*1j)) + npa.sum(fields_2_y[32,:]*(npa.real(fields_2_y_h[32,:])-npa.imag(fields_2_y_h[32,:])*1j)) - npa.sum(fields_4_x[32,:]*(npa.real(fields_4_x_h[32,:])-npa.imag(fields_4_x_h[32,:])*1j)) + npa.sum(fields_4_y[32,:]*(npa.real(fields_4_y_h[32,:])-npa.imag(fields_4_y_h[32,:])*1j))
                green_4 = -npa.sum(fields_2_x[34,:]*(npa.real(fields_2_x_h[34,:])-npa.imag(fields_2_x_h[34,:])*1j)) + npa.sum(fields_2_y[34,:]*(npa.real(fields_2_y_h[34,:])-npa.imag(fields_2_y_h[34,:])*1j)) - npa.sum(fields_4_x[34,:]*(npa.real(fields_4_x_h[34,:])-npa.imag(fields_4_x_h[34,:])*1j)) + npa.sum(fields_4_y[34,:]*(npa.real(fields_4_y_h[34,:])-npa.imag(fields_4_y_h[34,:])*1j))

                CT_green_0 = -npa.sum(fields_2_x[6,:]*(npa.real(fields_2_x_h[6,:])-npa.imag(fields_2_x_h[6,:])*1j)) + npa.sum(fields_2_y[6,:]*(npa.real(fields_2_y_h[6,:])-npa.imag(fields_2_y_h[6,:])*1j)) - npa.sum(fields_4_x[6,:]*(npa.real(fields_4_x_h[6,:])-npa.imag(fields_4_x_h[6,:])*1j)) + npa.sum(fields_4_y[6,:]*(npa.real(fields_4_y_h[6,:])-npa.imag(fields_4_y_h[6,:])*1j))
                CT_green_1 = -npa.sum(fields_2_x[8,:]*(npa.real(fields_2_x_h[8,:])-npa.imag(fields_2_x_h[8,:])*1j)) + npa.sum(fields_2_y[8,:]*(npa.real(fields_2_y_h[8,:])-npa.imag(fields_2_y_h[8,:])*1j)) - npa.sum(fields_4_x[8,:]*(npa.real(fields_4_x_h[8,:])-npa.imag(fields_4_x_h[8,:])*1j)) + npa.sum(fields_4_y[8,:]*(npa.real(fields_4_y_h[8,:])-npa.imag(fields_4_y_h[8,:])*1j))
                CT_green_2 = -npa.sum(fields_2_x[10,:]*(npa.real(fields_2_x_h[10,:])-npa.imag(fields_2_x_h[10,:])*1j)) + npa.sum(fields_2_y[10,:]*(npa.real(fields_2_y_h[10,:])-npa.imag(fields_2_y_h[10,:])*1j)) - npa.sum(fields_4_x[10,:]*(npa.real(fields_4_x_h[10,:])-npa.imag(fields_4_x_h[10,:])*1j)) + npa.sum(fields_4_y[10,:]*(npa.real(fields_4_y_h[10,:])-npa.imag(fields_4_y_h[10,:])*1j))
                CT_green_3 = -npa.sum(fields_2_x[12,:]*(npa.real(fields_2_x_h[12,:])-npa.imag(fields_2_x_h[12,:])*1j)) + npa.sum(fields_2_y[12,:]*(npa.real(fields_2_y_h[12,:])-npa.imag(fields_2_y_h[12,:])*1j)) - npa.sum(fields_4_x[12,:]*(npa.real(fields_4_x_h[12,:])-npa.imag(fields_4_x_h[12,:])*1j)) + npa.sum(fields_4_y[12,:]*(npa.real(fields_4_y_h[12,:])-npa.imag(fields_4_y_h[12,:])*1j))
                CT_green_4 = -npa.sum(fields_2_x[14,:]*(npa.real(fields_2_x_h[14,:])-npa.imag(fields_2_x_h[14,:])*1j)) + npa.sum(fields_2_y[14,:]*(npa.real(fields_2_y_h[14,:])-npa.imag(fields_2_y_h[14,:])*1j)) - npa.sum(fields_4_x[14,:]*(npa.real(fields_4_x_h[14,:])-npa.imag(fields_4_x_h[14,:])*1j)) + npa.sum(fields_4_y[14,:]*(npa.real(fields_4_y_h[14,:])-npa.imag(fields_4_y_h[14,:])*1j))
                CT_green_5 = -npa.sum(fields_2_x[46,:]*(npa.real(fields_2_x_h[46,:])-npa.imag(fields_2_x_h[46,:])*1j)) + npa.sum(fields_2_y[46,:]*(npa.real(fields_2_y_h[46,:])-npa.imag(fields_2_y_h[46,:])*1j)) - npa.sum(fields_4_x[46,:]*(npa.real(fields_4_x_h[46,:])-npa.imag(fields_4_x_h[46,:])*1j)) + npa.sum(fields_4_y[46,:]*(npa.real(fields_4_y_h[46,:])-npa.imag(fields_4_y_h[46,:])*1j))
                CT_green_6 = -npa.sum(fields_2_x[48,:]*(npa.real(fields_2_x_h[48,:])-npa.imag(fields_2_x_h[48,:])*1j)) + npa.sum(fields_2_y[48,:]*(npa.real(fields_2_y_h[48,:])-npa.imag(fields_2_y_h[48,:])*1j)) - npa.sum(fields_4_x[48,:]*(npa.real(fields_4_x_h[48,:])-npa.imag(fields_4_x_h[48,:])*1j)) + npa.sum(fields_4_y[48,:]*(npa.real(fields_4_y_h[48,:])-npa.imag(fields_4_y_h[48,:])*1j))
                CT_green_7 = -npa.sum(fields_2_x[50,:]*(npa.real(fields_2_x_h[50,:])-npa.imag(fields_2_x_h[50,:])*1j)) + npa.sum(fields_2_y[50,:]*(npa.real(fields_2_y_h[50,:])-npa.imag(fields_2_y_h[50,:])*1j)) - npa.sum(fields_4_x[50,:]*(npa.real(fields_4_x_h[50,:])-npa.imag(fields_4_x_h[50,:])*1j)) + npa.sum(fields_4_y[50,:]*(npa.real(fields_4_y_h[50,:])-npa.imag(fields_4_y_h[50,:])*1j))
                CT_green_8 = -npa.sum(fields_2_x[52,:]*(npa.real(fields_2_x_h[52,:])-npa.imag(fields_2_x_h[52,:])*1j)) + npa.sum(fields_2_y[52,:]*(npa.real(fields_2_y_h[52,:])-npa.imag(fields_2_y_h[52,:])*1j)) - npa.sum(fields_4_x[52,:]*(npa.real(fields_4_x_h[52,:])-npa.imag(fields_4_x_h[52,:])*1j)) + npa.sum(fields_4_y[52,:]*(npa.real(fields_4_y_h[52,:])-npa.imag(fields_4_y_h[52,:])*1j))
                CT_green_9 = -npa.sum(fields_2_x[54,:]*(npa.real(fields_2_x_h[54,:])-npa.imag(fields_2_x_h[54,:])*1j)) + npa.sum(fields_2_y[54,:]*(npa.real(fields_2_y_h[54,:])-npa.imag(fields_2_y_h[54,:])*1j)) - npa.sum(fields_4_x[54,:]*(npa.real(fields_4_x_h[54,:])-npa.imag(fields_4_x_h[54,:])*1j)) + npa.sum(fields_4_y[54,:]*(npa.real(fields_4_y_h[54,:])-npa.imag(fields_4_y_h[54,:])*1j))

                blue_0 = -npa.sum(fields_3_x[6,:]*(npa.real(fields_3_x_h[6,:])-npa.imag(fields_3_x_h[6,:])*1j)) + npa.sum(fields_3_y[6,:]*(npa.real(fields_3_y_h[6,:])-npa.imag(fields_3_y_h[6,:])*1j))
                blue_1 = -npa.sum(fields_3_x[8,:]*(npa.real(fields_3_x_h[8,:])-npa.imag(fields_3_x_h[8,:])*1j)) + npa.sum(fields_3_y[8,:]*(npa.real(fields_3_y_h[8,:])-npa.imag(fields_3_y_h[8,:])*1j))
                blue_2 = -npa.sum(fields_3_x[10,:]*(npa.real(fields_3_x_h[10,:])-npa.imag(fields_3_x_h[10,:])*1j)) + npa.sum(fields_3_y[10,:]*(npa.real(fields_3_y_h[10,:])-npa.imag(fields_3_y_h[10,:])*1j))
                blue_3 = -npa.sum(fields_3_x[12,:]*(npa.real(fields_3_x_h[12,:])-npa.imag(fields_3_x_h[12,:])*1j)) + npa.sum(fields_3_y[12,:]*(npa.real(fields_3_y_h[12,:])-npa.imag(fields_3_y_h[12,:])*1j))
                blue_4 = -npa.sum(fields_3_x[14,:]*(npa.real(fields_3_x_h[14,:])-npa.imag(fields_3_x_h[14,:])*1j)) + npa.sum(fields_3_y[14,:]*(npa.real(fields_3_y_h[14,:])-npa.imag(fields_3_y_h[14,:])*1j))

                CT_blue_0 = -npa.sum(fields_3_x[26,:]*(npa.real(fields_3_x_h[26,:])-npa.imag(fields_3_x_h[26,:])*1j)) + npa.sum(fields_3_y[26,:]*(npa.real(fields_3_y_h[26,:])-npa.imag(fields_3_y_h[26,:])*1j))
                CT_blue_1 = -npa.sum(fields_3_x[28,:]*(npa.real(fields_3_x_h[28,:])-npa.imag(fields_3_x_h[28,:])*1j)) + npa.sum(fields_3_y[28,:]*(npa.real(fields_3_y_h[28,:])-npa.imag(fields_3_y_h[28,:])*1j))
                CT_blue_2 = -npa.sum(fields_3_x[30,:]*(npa.real(fields_3_x_h[30,:])-npa.imag(fields_3_x_h[30,:])*1j)) + npa.sum(fields_3_y[30,:]*(npa.real(fields_3_y_h[30,:])-npa.imag(fields_3_y_h[30,:])*1j))
                CT_blue_3 = -npa.sum(fields_3_x[32,:]*(npa.real(fields_3_x_h[32,:])-npa.imag(fields_3_x_h[32,:])*1j)) + npa.sum(fields_3_y[32,:]*(npa.real(fields_3_y_h[32,:])-npa.imag(fields_3_y_h[32,:])*1j))
                CT_blue_4 = -npa.sum(fields_3_x[34,:]*(npa.real(fields_3_x_h[34,:])-npa.imag(fields_3_x_h[34,:])*1j)) + npa.sum(fields_3_y[34,:]*(npa.real(fields_3_y_h[34,:])-npa.imag(fields_3_y_h[34,:])*1j))
                CT_blue_5 = -npa.sum(fields_3_x[46,:]*(npa.real(fields_3_x_h[46,:])-npa.imag(fields_3_x_h[46,:])*1j)) + npa.sum(fields_3_y[46,:]*(npa.real(fields_3_y_h[46,:])-npa.imag(fields_3_y_h[46,:])*1j))
                CT_blue_6 = -npa.sum(fields_3_x[48,:]*(npa.real(fields_3_x_h[48,:])-npa.imag(fields_3_x_h[48,:])*1j)) + npa.sum(fields_3_y[48,:]*(npa.real(fields_3_y_h[48,:])-npa.imag(fields_3_y_h[48,:])*1j))
                CT_blue_7 = -npa.sum(fields_3_x[50,:]*(npa.real(fields_3_x_h[50,:])-npa.imag(fields_3_x_h[50,:])*1j)) + npa.sum(fields_3_y[50,:]*(npa.real(fields_3_y_h[50,:])-npa.imag(fields_3_y_h[50,:])*1j))
                CT_blue_8 = -npa.sum(fields_3_x[52,:]*(npa.real(fields_3_x_h[52,:])-npa.imag(fields_3_x_h[52,:])*1j)) + npa.sum(fields_3_y[52,:]*(npa.real(fields_3_y_h[52,:])-npa.imag(fields_3_y_h[52,:])*1j))
                CT_blue_9 = -npa.sum(fields_3_x[54,:]*(npa.real(fields_3_x_h[54,:])-npa.imag(fields_3_x_h[54,:])*1j)) + npa.sum(fields_3_y[54,:]*(npa.real(fields_3_y_h[54,:])-npa.imag(fields_3_y_h[54,:])*1j))
...
####################################################################################################
    ...
    if ML:
        ...
        def f(v, cur_beta):
            ...
            # Apply mapping
            x = Mapping.mapping(v,                             # design variables
                                eta_i,                         # threshold point of the linear filter
                                cur_beta,                      # binarization parameter
                                Nx,                            # number of design variables in x direction
                                Ny,                            # number of design variables in y direction
            ...
            if True: ##### compute objective and gradient ############################################################################################
                ...
                if v.size > 0:
                    gradient = v*0
                    gradient[:] = tensor_jacobian_product(Mapping.mapping, 0)(
                        v, 
                        eta_i, 
                        cur_beta,
                        Nx,                            
            ...
            if True: ##### Save the structure of the current iteration #################################################################################
                np.savetxt("./" + directory_name+"/structure_"+str(numevl) +".txt", design_variables.weights)
    ...
    if True: ##### Optimization ############################################################################################################################
        if ML:
            ...
            n = Nx * Ny * Nz  # number of design variables
    ...
    if True: ##### Save result plot ########################################################################################################################
        ...
        if ML:
            ...
            # Save the last design variables
            np.savetxt("./" + directory_name+"/lastdesign.txt", design_variables.weights)
