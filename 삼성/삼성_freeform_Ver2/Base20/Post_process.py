...
for i in x_list:
    ...
    if simcondition:  
        ...
        if True: #####  Set geometry  ################################################################################################################################
            # structure load
            structure_weight = np.loadtxt('structure_400.txt')
            ...
            # structure_weight = mpa.tanh_projection(structure_weight, np.inf, 0.5)
    ...
    if Opticaleff:
        ...
        if i == 0:
            source = [mp.Source(src, component=mp.Ex, size=source_size, center=source_center, amplitude=1)]
        elif i == 1:
            source = [mp.Source(src, component=mp.Ey, size=source_size, center=source_center, amplitude=1)]
        elif i == 2:
            source = [mp.Source(src, component=mp.Ex, size=source_size, center=source_center, amplitude=1),mp.Source(src, component=mp.Ey, size=source_size, center=source_center, amplitude=1)]
        opt.sim.change_sources(source)
    ...
    if layerplot:
        ...
        plt.gca().axes.xaxis.set_visible(False)
        plt.gca().axes.yaxis.set_visible(False)
        plt.savefig("./"+directory_name+"/Design/design_layer.png")
        plt.cla()   # clear the current axes
        plt.clf()   # clear the current figure
        plt.close() # closes the current figure
    ###############################################################################################################################
