...
def multiregion_mapping(
    ...
    x_copy = (x.reshape(Nx * Ny, 4)).transpose()
    ...
    x2 = ((x2.reshape(4, Nx*Ny)).transpose()).flatten() # reshape

    x2 = mpa.tanh_projection(x2, beta, eta) # projection
    
    x2 = (x2.reshape(Nx * Ny, 4)).transpose()
    ...
    stacked_x2 = npa.concatenate([x_0, x_1, x_2, x_3], axis=0)

    if ind==0:
        return x_0, x_1, x_2, x_3
    elif ind==1:
        return stacked_x2.reshape(4, Nx*Ny).transpose().flatten()
#----------------------------------------------------------------------------------------------------------------------------------
