...
while outer_iter < (num_betas + 1):
    ...
    counter = 0
    lr_counter = 0
    EPS_Step = 0.2/(2**outer_iter)      
    while inner_iter < Max_iter:
        FoM, x, Geo= f(x, cur_beta, EPS_Step) # FoM, sliced geo, total geo

        print(np.max(Geo))
        print(np.min(Geo))
        print(f"Learning rate: {EPS_Step}")

        if FoM < FoM_best * 1.01: # 5% improvement?
            lr_counter +=1
            if lr_counter == 4: # update the learning rate /10
                EPS_Step *= 0.1
            if lr_counter > 3: # No improvement after lr update 
                counter +=1
            if counter == 2: # Inner stop condition
                np.savetxt(f"{design_dir}Geometry_iter{numevl}_lr{EPS_Step}_beta{cur_beta}.txt", design_variables.weights)
