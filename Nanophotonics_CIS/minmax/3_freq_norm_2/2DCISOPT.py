...
FourierFields_01= mpa.FourierFields(sim0,mp.Volume(center= mp.Vector3(0, Sy/ 2 - Lpml - Sourcespace -0.02, 0),size=source_size),mp.Ez,yee_grid=True)
FourierFields_11= mpa.FourierFields(sim1,mp.Volume(center= mp.Vector3(0, Sy/ 2 - Lpml - Sourcespace -0.02, 0),size=source_size),mp.Ez,yee_grid=True)
FourierFields_12= mpa.FourierFields(sim2,mp.Volume(center= mp.Vector3(0, Sy/ 2 - Lpml - Sourcespace -0.02, 0),size=source_size),mp.Ez,yee_grid=True)

ob_list0 = [FourierFields_0, FourierFields_01]
ob_list1 = [FourierFields_1, FourierFields_11]
ob_list2 = [FourierFields_2, FourierFields_12]
...
def J_0(fields, source):
    return - (npa.abs(fields[:,1]) ** 2)/ (npa.abs(source[:,1]) ** 2)
def J_1(fields, source):
    return - (npa.abs(fields[:,1]) ** 2)/ (npa.abs(source[:,1]) ** 2)
def J_2(fields, source):
    return - (npa.abs(fields[:,1]) ** 2)/ (npa.abs(source[:,1]) ** 2)
