...
d_values = [0.5, 1, 1.5, 2, 2.5, 5, 10];

for diameter = d_values

    d=diameter/2.0; 
    focal = d * sqrt(21.0) / 2.0;
    r_cur = focal / 2 ;

    Lx = diameter + Lpml * 2;
    Ly = Lpml + 0.25 * d + focal + r_cur - sqrt(r_cur * r_cur - d * d) + 0.5 * d + Lpml;
    X = round(rs * (Lx - 2 * Lpml));
    Y = round(rs * (Ly - 2 * Lpml));

    % 빨간색       
    Hx_r = sprintf('%.1fHx_field.h5', diameter);  % d 값을 이용하여 문자열 포맷팅
    hinfo_Hx_r = hdf5info(Hx_r);  % 포맷팅된 파일명을 hdf5info 함수에 전달

    Hy_r = sprintf('%.1fHy_field.h5', diameter);  % d 값을 이용하여 문자열 포맷팅
    hinfo_Hy_r = hdf5info(Hy_r);  % 포맷팅된 파일명을 hdf5info 함수에 전달

    Ez_r = sprintf('%.1fEz_field.h5', diameter);  % d 값을 이용하여 문자열 포맷팅
