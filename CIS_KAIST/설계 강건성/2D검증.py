...
for i in design:
    ...
    # Calculate crosstalk
    crosstalk_r_g = np.sum((Tg)[0:66])
    crosstalk_r_b = np.sum((Tb)[0:66])
    crosstalk_g_b = np.sum((Tb)[66:160]) 
    crosstalk_g_r = np.sum((Tr)[66:160]) 
    crosstalk_b_r = np.sum((Tr)[160:300]) 
    crosstalk_b_g = np.sum((Tg)[160:300]) 
