...
# In[1]:
#First FoM: 4.205660321315278
#Current FoM: 107.62091230063109

#Elapsed run time = 5054.8247 s
...
FourierFields_1 = mpa.FourierFields(sim,mp.Volume(center=monitor_position_1,size=monitor_size_1),mp.Ey,yee_grid=True)

FourierFields_2 = mpa.FourierFields(sim,mp.Volume(center=monitor_position_2,size=monitor_size_2),mp.Ey,yee_grid=True)

FourierFields_3 = mpa.FourierFields(sim,mp.Volume(center=monitor_position_3,size=monitor_size_3),mp.Ey,yee_grid=True)

ob_list = [FourierFields_0, FourierFields_1, FourierFields_2,FourierFields_3]
...
def J_0(fields_0, fields_1, fields_2,fields_3):
    return npa.mean(npa.abs(fields_0[2,:]) ** 2) + npa.mean(npa.abs(fields_1[1,:]) ** 2) + npa.mean(npa.abs(fields_2[0,:]) ** 2) + npa.mean(npa.abs(fields_3[1,:]) ** 2) 
...
intensities_1 = np.abs(opt.get_objective_arguments()[1][:,1]) ** 2
plt.figure(figsize=(10, 8))
# plt.figure()
...
intensities_2 = np.abs(opt.get_objective_arguments()[2][:,1]) ** 2
plt.figure(figsize=(10, 8))
# plt.figure()
...
intensities_3 = np.abs(opt.get_objective_arguments()[3][:,1]) ** 2
plt.figure(figsize=(10, 8))
# plt.figure()
