...
def c(result, x, gradient, beta):
    ...
    evaluation_history.append([np.max(np.real(f0_0)), np.max(np.real(f0_1))])

    numevl += 1
    cur_iter[0] = cur_iter[0] + 1

    print("First FoM: {}".format(evaluation_history[0]))
    print("Current f0_0: {}, f0_1: {}".format(np.real(f0_0), np.real(f0_1)))
